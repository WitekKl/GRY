...
class MenuView(arcade.View):
    ...
    def __init__(self):
        ...
        self.manager.enable()
        arcade.load_font(font_name="pymunk/Risaltyp.ttf")
...
class IntroView(arcade.View):
    ...
    def on_show(self):
        ...
        self.background = arcade.load_texture("pymunk/images/instr.jpg")
...
class GameOverView(arcade.View):
    ...
    def on_draw(self):
        ...
        if not self.window.poziomy:
            ...
            arcade.draw_text("1 - od poczatku, 2 - ten sam level", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 - 55,
                             arcade.color.RED_BROWN, font_size=46, anchor_x="center", font_name='Kenney Pixel')
        else:
            score_text1 = "Gratulacje, przeszedles wszystko, Twoj wynik : " + str(self.window.score)
            arcade.draw_text(score_text1, SCREEN_WIDTH / 2,
                             2 * SCREEN_HEIGHT / 3, arcade.csscolor.RED, 62, anchor_x="center",
                             font_name='Kenney Pixel')
            arcade.draw_text("Nacisnij klawisz myszki i poczekaj chwilke", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 - 55,
                             arcade.color.RED_BROWN, font_size=46, anchor_x="center", font_name='Kenney Pixel')
...
class GameView(arcade.View):
    ...
    def setup(self):
        # główna część gry
        arcade.load_font(font_name="pymunk/Kenney Pixel.ttf")
        self.zamiana_x = 0
        self.zamiana_y = 0
        self.zamiana_tekst = ""
        self.view_bottom = 0
    ...
    def on_update(self, delta_time):
        if not self.koniec:
            self.scene.update_animation(delta_time,
                                        ["Dynamic Items", "Coins", "Xenemy", "Background", "Extra_item", "Dont touch"])
            ...
            if len(self.scene.name_mapping["golem"]) == 0:
                self.nowy_golem()
    ...
    def wynik(self):
        ...
        self.zliczdead += 1
        if self.ilosczyc == 3:
            score_text = "Jeszcze raz - masz jeszcze " + str(self.ilosczyc - 1) + " zycia"
        elif self.ilosczyc == 2:
            score_text = "Jeszcze raz - masz jeszcze " + str(self.ilosczyc - 1) + " zycie"
        elif self.ilosczyc == 1:
            score_text = "niestety - porazka"
    ...
    def wynik2(self):
        ...
        score_text = f"Score: {self.window.score}" + "    Moc:" + str(self.mocshield) + "   Live:" + str(
            self.ilosczyc)
        arcade.draw_text(score_text, 10 + self.view_left,
                         30 + self.view_bottom, arcade.csscolor.BLACK, 24, font_name='Kenney Pixel')
        arcade.draw_texture_rectangle(400 + self.view_left,
                                      40 + self.view_bottom, 40, 40, self.scr2)
        arcade.draw_text(str(self.key), 425 + self.view_left,
                         30 + self.view_bottom, arcade.csscolor.BLACK, 24, font_name='Kenney Pixel')
        arcade.draw_scaled_texture_rectangle(500 + self.view_left,
                                             40 + self.view_bottom, self.scr1, scale=0.5)
        arcade.draw_text(str(self.diamond), 525 + self.view_left,
                         30 + self.view_bottom, arcade.csscolor.BLACK, 24, font_name='Kenney Pixel')
        arcade.draw_scaled_texture_rectangle(600 + self.view_left,
                                             40 + self.view_bottom, self.scr4, scale=0.25)
        arcade.draw_text(str(self.gun1), 620 + self.view_left,
                         30 + self.view_bottom, arcade.csscolor.BLACK, 24, font_name='Kenney Pixel')
        arcade.draw_scaled_texture_rectangle(700 + self.view_left,
                                             30 + self.view_bottom, self.scr3, scale=0.5)
        arcade.draw_text(str(self.gun2), 740 + self.view_left,
                         40 + self.view_bottom, arcade.csscolor.BLACK, 24, font_name='Kenney Pixel')
